...
const initializeCaseAudited = withAudit<InitializeCaseInput, InitializeCaseOutput>(
  ...
  async (ctx: TenantContext, input: InitializeCaseInput): Promise<InitializeCaseOutput> => {
    ...
    // 5. Execute creation within an authenticated transaction context (mandatory security correction)
    return await dbClient.transaction(async (tx) => {
      ...
      // Create Case
      const ptsCase = await caseRepo.createCase(input.patientId, 'active');
