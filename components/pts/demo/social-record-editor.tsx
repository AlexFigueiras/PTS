...
import { Home, Edit3, Check, X } from 'lucide-react';
...
export function SocialRecordEditor({ record }: { record: Record }) {
  ...
  function save() {
    startTransition(async () => {
      await updateSourceRecordAction({ id: record.id, rawText: text, source: 'social' });
      setSaved(true);
      setEditing(false);
    ...
    <div className="rounded-2xl border border-amber-500/20 bg-card p-6 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          {editing ? (
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl bg-amber-500/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-amber-600 transition hover:bg-amber-500/20"
            >
      ...
      {editing ? (
        <textarea
          ...
          rows={8}
          className="w-full resize-none rounded-xl border border-amber-500/30 bg-background/50 p-4 text-sm text-foreground outline-none focus:border-amber-500/60 focus:ring-2 focus:ring-amber-500/10"
        />
      ) : (
