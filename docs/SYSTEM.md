...
| `plan_tier` | `BASICO, PREMIUM` | `tenants.plan_tier` (gating Básico vs Premium; default `BASICO`, fail-closed) |
...
> [!WARNING]
> **Fronteira de Transação:** O wrapper `withTransactionContext` deve sempre envelopar a chamada à função de auditoria (`withAudit`), e não ser executado apenas internamente dentro da função passada para `withAudit`. Isso garante que a gravação final do log de auditoria ocorra no contexto da transação ativa e evite erros de violação de políticas RLS em repositórios com acesso direto ao banco restrito.
...
- **Cron de processamento**: Disparado periodicamente (ex: a cada minuto) via Vercel Cron.
- **Cron diário de descumprimento**: `/api/jobs/check-noncompliance` (GET/POST, protegido por `CRON_SECRET`) — enfileira um job `non_compliance_check` por tenant, executado 1×/dia (ex: `0 7 * * *`). Handler registrado no `JobProcessorService`. Ver §5.7 do plano e `modules/pts/services/non-compliance-alert.service.ts`.
- **Cron de monitoramento contínuo**: `/api/jobs/scan-ingestion` (GET/POST, protegido por `CRON_SECRET`) — enfileira um job `ingest_scan` por tenant; o handler reprocessa casos ativos com derivação de dimensões desatualizada via `runIngestionPipeline`. A IA analisa continuamente, sem clique manual. Ver `modules/pts/services/ingest-scan.service.ts`.
- **Cron de reavaliação**: `/api/jobs/check-reavaliacoes` (GET/POST, protegido por `CRON_SECRET`) — enfileira um job `reavaliacao_check` por tenant; o handler notifica o Técnico de Referência das ações cuja `data_proxima_reavaliacao` vence no dia. Ver `modules/pts/services/reavaliacao-reminder.service.ts`.
...
| **Bloco 1** | **Ciclo de vida temporal da Ação** — campos temporais obrigatórios (`prazo_fim`, `frequencia_tipo`, `data_proxima_reavaliacao`, `aceite_usuario`, `horizonte_tipo`) em `pts_actions` + tabela `pts_reavaliacoes` com histórico datado + gate Zod §5.6 + `ReavaliacaoService` + UI do painel de reavaliações em `action-list.tsx` | 🟢 Concluído (2026-06-02) |
| **Bloco 2** | **Semáforo + Alertas de Descumprimento** — `computeStatusTemporal` (puro, `@pts/domain`) + semáforo verde/amarelo/vermelho na UI + ordenação vermelho-primeiro + mig. 0029 (`signal_subtype` em `pts_signals`) + `NonComplianceAlertService` (marcos 1/2/3 meses, idempotente, cria `alerta_descumprimento` no motor existente + ação `busca_ativa_sugerida`) + notificação in-app + e-mail via Resend + `non_compliance_check` handler no `JobProcessorService` + cron `/api/jobs/check-noncompliance` (1×/dia, por tenant) | 🟢 Concluído (2026-06-02) |
| **Bloco 3** | **Classificação de risco por horizonte temporal (§5.8)** — mig. 0030 (`nivel_intensidade` enum em `pts_plans` + coluna `arquivado` em `pts_cases`). `NivelIntensidade` + `NIVEL_TRANSITIONS` + `NIVEL_CADENCIA_MESES` em `@pts/domain/temporal-action.ts`. `PtsPlanRepository.updateNivelIntensidade`, `PtsCaseRepository.archiveCase`. `IntensityLevelService` (valida transição FSM, exige RT, arquiva caso ao atingir `alta_continuidade`, sugere transição via sinalização `sugerida` quando todas as metas estão concluídas — humano-no-loop). Server Actions `transitionNivelIntensidadeAction` + `suggestNivelTransitionAction` (zero-trust). UI `IntensityPanel` + `NivelIntensidadeBadge` (`components/pts/intensity-panel.tsx`) exibidos na página do caso com gate de confirmação para RT. Dashboard `PROFESSIONAL` com split intensivo/manutenção na `CaseGrid`. | 🟢 Concluído (2026-06-02) |
| **Bloco 4** | **Protagonismo do Usuário (§5.9)** — mig. 0031 (novos enums `participacao_usuario` e `encontros_tipo` + colunas `participacao_usuario` e `participacao_justificativa` em `pts_plans` + tabela `encontros` com RLS e `tenant_id`). `PtsPlanRepository.updateParticipation`, `PtsEncontroRepository` (CRUD encontros). `CaseStatusService` (Gate de Ativação do Plano impedindo transição do caso para `pts_ativo` sem registro de participação de usuário ou sem justificativa de incapacidade). `EncontroService` (valida presenca obrigatória do usuário em `reuniao_pts`). `RecordActionService` (bloqueia status `pactuada`/`em_andamento` e redireciona ações recusadas/repactuadas para `bloqueada` com nota de repactuação). Server Actions zero-trust em `modules/pts/actions.ts`. UI components `EncontroModal` + `ParticipationAndEncontros` + indicador no header do caso. Testes de integração em `modules/pts/__tests__/user-protagonism.test.ts`. | 🟢 Concluído (2026-06-02) |
| **Bloco 5** | **Ajustes menores (roteamento, seletividade do PTS, documento) (§5.5, §6, §6B do plano)** — mig. 0032 (necessidade `comorbidade_clinica` em `network-catalog.ts` para Atenção Básica/eSF). Regra de comorbidades clínicas e físicas em paciente psíquico roteadas para eSF, não para o CAPS. IA com seletividade de indicação de PTS baseada em critérios explícitos de criticidade de caso (reinternações, risco). UI de Minuta Documental Legal (PIA/PTS formal) exibindo ficha de qualificação, editor de minuta e tabela de Compromissos Pactuados a partir de ações `pactuada`. | 🟢 Concluído (2026-06-02) |
| **Bloco 6 (Ajuste A)** | **Gating Básico vs Premium** — enum/coluna `plan_tier` em `tenants` (mig. 0033, default `BASICO`, fail-closed) + `planTier` opcional no `TenantContext` (carregado em `getTenantContext`) + helpers `hasTier`/`requireTier`/`TierError` em [authorization.ts](../lib/auth/authorization.ts). Gate canônico na ativação do plano ([case-status.service.ts](../modules/pts/services/case-status.service.ts), transição `pts_ativo`/`pia_ativo` exige PREMIUM) + defesa-em-profundidade nas server actions do ciclo PTS (`createActionAction`, `createReavaliacaoAction`, `createEncontroAction`, `transitionNivelIntensidadeAction`, `suggestNivelTransitionAction`, `updatePlanParticipationAction`). UI: a página do Caso esconde Intensidade/Participação/Ações e mostra aviso de capacidade Premium para tenants Básico; Sinalizações e Dimensões permanecem (núcleo Básico). | 🟢 Concluído (2026-06-08) |
| **Bloco 7 (Ajuste B)** | **Vínculo de matrícula + caixa de sugestões da IA** — mig. 0034: `municipal_registry` em `profiles` (matrícula funcional única, chave de vínculo com o sistema externo de origem) + `author_municipal_registry`/`origin_unit_id` em `source_health_records`/`source_social_records`. `fanOutSignals` resolve a matrícula → perfil e atribui a **autoria** da sinalização ao profissional que originou o relato (o gate `actorIsAuthor` da FSM faz a sugestão cair no perfil dele para aceitar/rejeitar); usa `origin_unit_id` como unidade de origem (habilita ingestão sem unidade ativa). `PtsSignalRepository.findSuggestedForAuthor`, rota `/inbox` + `SuggestionInbox` (aceitar=`confirmSignalAction`, rejeitar=`discardSignalAction`) e item "Sugestões" na sidebar. Fallback ao operador da ingestão quando não há matrícula vinculável. | 🟢 Concluído (2026-06-08) |
| **Bloco 8 (Ajuste C)** | **Monitoramento contínuo (IA agendada)** — pipeline de ingestão extraído para [ingestion-pipeline.service.ts](../lib/pts/ingestion-pipeline.service.ts) (`runIngestionPipeline`), reutilizado pela Server Action manual, pelo endpoint `POST /api/ingest` e pelo novo job (DRY; o endpoint passou a tokenizar/atribuir como a action). `IngestScanService.runIngestScan` (worker global, superusuário, padrão do `NonComplianceAlertService`) reprocessa casos ativos cujo registro-fonte é mais recente que a última derivação, re-rodando o pipeline. Handler `ingest_scan` no `JobProcessorService` + cron `/api/jobs/scan-ingestion` (GET/POST, `CRON_SECRET`) enfileira um job por tenant. Idempotente (dedupe de sinalização); `authorId` nullable evita crash quando não há matrícula vinculável. | 🟢 Concluído (2026-06-08) |
| **Bloco 9 (Ajuste D)** | **Maturidade do PTS** — quatro refinos: (1) **Radar de evolução** discoverável (link na página do Caso → `/pts/evolution`; `createPtsEvolution` e `EvolutionTracker` já existiam, 5 dimensões via `PTS_DOMAINS = DIMENSIONS`); (2) **Scheduler de reavaliação** — `ReavaliacaoReminderService` (fila `reavaliacao_check`, cron `/api/jobs/check-reavaliacoes`) notifica o RT quando `data_proxima_reavaliacao` vence; (3) **Reatribuição de RT** — `PtsPlanRepository.updateOwner` + `ReassignRtService` (withAudit, MANAGER+Premium, valida membro) + `reassignReferenceTechnicianAction` + `ReassignRtControl` na página do Caso; (4) **Atas de encontro** — mig. 0035 (`ata` em `encontros`) + campo no `EncontroModal` e exibição em `ParticipationAndEncontros`. | 🟢 Concluído (2026-06-08) |
...
| 2026-06-02 | **Bloco 1 — Ciclo de vida temporal da Ação (§5.6 do plano)**: mig. 0028 (`data_inicio`, `prazo_fim`, `frequencia_tipo`, `frequencia_detalhe`, `proximo_retorno`, `data_proxima_reavaliacao`, `horizonte_tipo`, `aceite_usuario` em `pts_actions` + tabela `pts_reavaliacoes` com RLS). Tipos em `@pts/domain/temporal-action.ts`. Gate Zod §5.6 (bloqueia pactuação sem prazo fechado/frequência/reavaliação). `PtsReavaliacaoRepository`, `ReavaliacaoService`, `createReavaliacaoAction`/`listReavaliacoesAction`. UI `action-list.tsx` com formulário expandido de Nova Ação e painel inline de Reavaliações com histórico datado e registro de nova reavaliação. |
| 2026-06-02 | **Bloco 2 — Semáforo + Alertas de Descumprimento (§5.7 do plano)**: `computeStatusTemporal` + `monthsOverdue` + `StatusTemporal` (puro, sem I/O) em `@pts/domain/temporal-action.ts`. Mig. 0029: coluna `signal_subtype` (`alerta_descumprimento` / `busca_ativa_sugerida`) em `pts_signals` + enum PG + índice. `SignalSubtype` em `@pts/domain/signal.ts`. `PtsSignalRepository.create` aceita `signalSubtype`. UI `action-list.tsx`: `SemaforoBadge` (dot colorido + label), card com borda colorida por semáforo, ordenação vermelho→amarelo→verde. `NonComplianceAlertService` (`modules/pts/services/`): cron-safe, idempotente por `(actionId, alerta aberto)`, cria sinalização `alerta_descumprimento` + ação `busca_ativa_sugerida` nos marcos 1/2/3 meses, notificação in-app + e-mail via Resend. Handler `non_compliance_check` registrado em `JobProcessorService`. Rota `/api/jobs/check-noncompliance` (GET+POST, `CRON_SECRET`) enfileira um job por tenant, 1×/dia. |
| 2026-06-02 | **Bloco 3 — Classificação de risco por horizonte temporal (§5.8 do plano)**: Mig. 0030: enum `nivel_intensidade` (`intensivo`/`manutencao_semestral`/`manutencao_anual`/`alta_continuidade`) + coluna em `pts_plans` (default `intensivo`) + coluna `arquivado boolean` em `pts_cases` + índices. Tipos `NivelIntensidade` + `NIVEL_TRANSITIONS` + `NIVEL_CADENCIA_MESES` + `NIVEL_INTENSIDADE_LABELS` em `@pts/domain/temporal-action.ts`. `PtsPlanRepository.updateNivelIntensidade`, `PtsCaseRepository.archiveCase`. `IntensityLevelService`: valida FSM de transição, exige RT, arquiva caso ao atingir `alta_continuidade`, método `suggestTransitionIfEligible` cria sinalização `sugerida` quando todas as metas ativas == 0 (humano-no-loop). Server Actions `transitionNivelIntensidadeAction` + `suggestNivelTransitionAction` em `modules/pts/actions.ts`. UI `IntensityPanel` + `NivelIntensidadeBadge` em `components/pts/intensity-panel.tsx` injetados na página `/patients/[id]/caso`. Dashboard PROFESSIONAL `CaseGrid` splitado em fila **Intensivo** (destaque, reavaliação mensal) e fila **Manutenção** (opacidade reduzida, desafogo visual), com badge de nível por caso. |
| 2026-06-02 | **Bloco 4 — Protagonismo do Usuário (§5.9 do plano)**: mig. 0031 (encontros + campos de participação do usuário em `pts_plans`). Repositório `PtsEncontroRepository` e métodos de participação no `PtsPlanRepository`. Serviços `CaseStatusService` (Gate de Ativação), `EncontroService` (presença obrigatória) e `RecordActionService` (travas de status de aceite). UI componentes `EncontroModal`, `ParticipationAndEncontros`, e integração completa na página do Caso (/caso/page.tsx) com indicador de progresso no header. Suíte de testes `user-protagonism.test.ts` validando todas as regras intersetoriais de protagonismo. |
| 2026-06-02 | **Bloco 5 — Ajustes menores (§5.5, §6, §6B do plano)**: mig. 0032 (necessidade `comorbidade_clinica` em `network_components`). Regra de roteamento de comorbidades físicas para Atenção Básica/eSF e seletividade de indicação de PTS na IA baseada em criticidade. Interface e rota de geração de Minuta Documental Legal (`/patients/[id]/documento`) com editor integrado e tabela dinâmica de Compromissos Pactuados filtrando ações `pactuada` com suporte a impressão. |
| 2026-06-08 | **Exposição do Mapa Estratégico**: Criada rota `/mapa` consultando cidadãos geolocalizados (`lat` e `lon` preenchidos) e exibindo o componente `PatientMap` integrado com a rede de serviços públicos (`PUBLIC_SERVICES`). Nova opção "Mapa" inserida no menu de navegação lateral (`AppSidebar`). |
| 2026-06-08 | **Gating Básico/Premium (Ajuste A — empacotamento de duas frentes)**: tier por município (`plan_tier` em `tenants`, mig. 0033, default `BASICO`/fail-closed) trancando o ciclo PTS/PIA na ativação do plano, com camada de entitlement (`hasTier`/`requireTier`/`TierError`), `planTier` no `TenantContext`, defesa-em-profundidade nas server actions do ciclo e gating de UI na página do Caso. Fronteira: **Básico** = monitoramento + dimensões + sinalizações/encaminhamentos (caso até `acompanhamento`); **Premium** = ciclo PTS/PIA (ativar plano, RT, encontros, metas, reavaliações). |
| 2026-06-08 | **Vínculo de matrícula + caixa de sugestões (Ajuste B)**: `profiles.municipal_registry` (matrícula funcional única) + atribuição de origem (`author_municipal_registry`/`origin_unit_id`) nas tabelas-fonte (mig. 0034). O fan-out passa a atribuir a autoria da sinalização derivada ao profissional que escreveu o relato na origem (resolvido pela matrícula), de modo que a sugestão cai no perfil dele; rota dedicada `/inbox` (`SuggestionInbox`) para o profissional aceitar/rejeitar (`confirmSignalAction`/`discardSignalAction`), com fallback ao operador da ingestão quando não há matrícula vinculável. |
| 2026-06-08 | **Monitoramento contínuo / IA agendada (Ajuste C)**: pipeline de ingestão extraído para `runIngestionPipeline` (compartilhado por `runIngestAction`, `POST /api/ingest` e o job — DRY). `runIngestScan(tenantId)` (worker global) reprocessa casos ativos com dimensões desatualizadas; handler `ingest_scan` no `JobProcessorService` + cron `/api/jobs/scan-ingestion` enfileira um job por tenant. A IA passa a analisar continuamente conforme chegam relatos, sem clique manual. |
| 2026-06-08 | **Maturidade do PTS (Ajuste D)**: Radar de evolução discoverável (link na página do Caso; feature já existente, 5 dimensões); `ReavaliacaoReminderService` + cron `/api/jobs/check-reavaliacoes` (lembrete de reavaliação ao RT); reatribuição de Técnico de Referência (`PtsPlanRepository.updateOwner` + `ReassignRtService` auditado + `ReassignRtControl`); atas de encontro (mig. 0035, campo `ata` no modal e na lista). |
